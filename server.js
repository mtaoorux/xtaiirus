...
function gitCommitAndPush() {
    try {
        ...
        execSync('git config user.email "mixvibe-bot@users.noreply.github.com"', { stdio: 'pipe' });
        execSync('git config user.name "MixVibe Bot"', { stdio: 'pipe' });
        execSync('git add data.json', { stdio: 'pipe' });
        execSync(`git commit -m "Update data.json – ${timestamp}"`, { stdio: 'pipe' });
        execSync('git push origin main', { stdio: 'pipe' });
        console.log('✅ Data committed and pushed to GitHub');
        return true;
    } catch (e) {
                            ...
                            if (t.success && t.data?.length) {
                                ...
                                for (const tp of tops) {
                                    for (const type of ['videos', 'notes', 'dpp']) {
                                        try {
                                            const c = await call(`/api/content?batchId=${b._id}&subjectId=${s._id}&topicId=${tp._id}&contentType=${type}`);
                                            if (c.success && c.data?.length) data.content[`${b._id}_${s._id}_${tp._id}_${type}`] = c;
...
const server = http.createServer((req, res) => {
    ...
    if (p === '/api/health') return json(res, { status: 'ok', domain: DOMAIN, uptime: Math.floor((Date.now() - started) / 1000), batches: data.batches.batches.length, extracting, storage: 'GitHub Repo' });
    ...
    if (p === '/' || p === '/admin') return page(res, homePage());
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .btn{padding:.8rem 1.5rem;border:none;border-radius:6px;cursor:pointer;font-weight:600;font-size:.9rem}
        ...
        .copy-btn{cursor:pointer;background:#21262d;border:1px solid #30363d;color:#c9d1d9;padding:.3rem .8rem;border-radius:4px;font-size:.75rem;margin-left:8px}
        .footer{text-align:center;padding:2rem;color:#484f58;font-size:.8rem}
        pre{background:#0d1117;padding:1rem;border-radius:4px;margin-top:.5rem;max-height:200px;overflow:auto;font-size:.8rem;display:none}
    </style>
</head>
<body>
<div class="header"><h1>🔴 MixVibe Mirror</h1><span style="color:#8b949e;font-size:.8rem">${domain}</span></div>
<div class="container">
    <div class="card">
        <h3>🔄 Extraction</h3>
    ...
    <div class="card">
        ...
        <ul class="endpoints">
            ...
            <li><span class="method GET">GET</span> <code>/api/live?batchId=</code></li>
            <li><span class="method GET">GET</span> <code>/api/topics?batchId=&subjectId=</code></li>
            <li><span class="method GET">GET</span> <code>/api/content?batchId=&subjectId=&topicId=&contentType=</code></li>
            <li><span class="method GET">GET</span> <code>/api/stats</code></li>
            <li><span class="method POST">POST</span> <code>/api/extract</code></li>
        </ul>
    </div>
</div>
<div class="footer">MixVibe Mirror · Data stored in GitHub repo</div>
<script>
async function extract(){const b=document.getElementById('btn');b.disabled=true;b.textContent='Starting...';try{await fetch('/api/extract',{method:'POST'})}catch(e){}setTimeout(()=>{b.disabled=false;b.textContent='Start Extraction'},3000)}
async function poll(){try{const r=await fetch('/api/stats');const d=await r.json();document.getElementById('v1').textContent=d.counts.batches;document.getElementById('v2').textContent=d.counts.subjects;document.getElementById('v3').textContent=d.counts.topics;document.getElementById('v4').textContent=d.counts.content;document.getElementById('v5').textContent=d.counts.live;const s=document.getElementById('status');const bar=document.getElementById('bar');if(d.extraction?.running){s.className='running';const pct=d.extraction.total>0?Math.round((d.extraction.done/d.extraction.total)*100):0;bar.style.width=pct+'%';s.innerHTML='Extracting: '+d.extraction.done+'/'+d.extraction.total+' – '+d.extraction.batch}else{s.className='';bar.style.width='0%';s.innerHTML=d.meta?.updated?'✅ Last updated: '+new Date(d.meta.updated).toLocaleString():'No data yet'}}catch(e){}}
...
if (fs.existsSync(DATA_FILE)) {
    ...
    catch (e) { log('Error loading data, starting fresh'); saveToFile(); }
} else { saveToFile(); }

server.listen(PORT, () => {
    console.log(`\n🚀 MixVibe Mirror: https://${DOMAIN}\n   Port: ${PORT}\n   Storage: GitHub Repo (data.json)\n`);
});
